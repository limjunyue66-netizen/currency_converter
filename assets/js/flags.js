/**
 * Currency to ISO 3166-1 alpha-2 country code mapping for flag icons.
 * Uses flagcdn.com (free CDN, no API key required).
 */
const Flags = {
    /** Map currency codes to country/region codes for flags */
    currencyToCountry: {
    ...
    getCountryCode(currencyCode) {
        return this.currencyToCountry[(currencyCode || '').toUpperCase()] || 'un';
    },

    getUrl(currencyCode, size = 40) {
        const cc = this.getCountryCode(currencyCode);
        const w = size <= 20 ? 20 : size <= 40 ? 40 : 80;
        return `https://flagcdn.com/w${w}/${cc}.png`;
    },

    imgHtml(currencyCode, size = 24, className = 'flag-icon') {
        const code = (currencyCode || '').toUpperCase();
        const cc = this.getCountryCode(code);
        const w = size <= 20 ? 20 : size <= 40 ? 40 : 80;
        return `<img class="${className}" src="https://flagcdn.com/w${w}/${cc}.png" width="${size}" height="${Math.round(size * 0.75)}" alt="${code} flag" loading="lazy">`;
    },
};
